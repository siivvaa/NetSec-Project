...
class MD5State:
    def __init__(self):
        self.length: int = 0
        self.state: tuple[int, int, int, int] = (0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476)   #state variable that is finally going to be made into a digest
        self.n_filled_bytes: int = 0
        self.buf: bytearray = bytearray(md5_block_size)

    def digest(self) -> bytes:
        return b''.join(x.to_bytes(length=4, byteorder='little') for x in self.state)           # state variables
    ...
    def process(self, stream: BinaryIO) -> None:
        ...
        while bytes_read := stream.read(md5_block_size - self.n_filled_bytes):
            ...
            else:                                            # if there were already some bytes in the buffer or if it did not read enough bytes to fill the buffer
    ...
    def finalize(self) -> None:                             # padding
        ...
        if self.n_filled_bytes + n_bytes_needed_for_len > md5_block_size:
            ...
            self.compress(self.buf)                             #extra compression if adding te length could overflow the block size
            self.n_filled_bytes = 0
    ...
    def compress(self, msg_chunk: bytearray) -> None:               # md5 operates on chunks of 512 bits
        assert len(msg_chunk) == md5_block_size  # 64 bytes, 512 bits
        msg_ints = [int.from_bytes(msg_chunk[i:i + 4], byteorder='little') for i in range(0, md5_block_size, 4)]      # convert those bytes to integers
        assert len(msg_ints) == 16      #64 bytes to 16 integers

        a, b, c, d = self.state                 #state variables

        for i in range(md5_block_size):         # 64 steps
            bit_mixer = mixer_for_step[i]       # there is a mixer for each of the 64 steps
            msg_idx = msg_idx_for_step[i]
            a = (a + bit_mixer(b, c, d) + msg_ints[msg_idx] + sine_randomness[i]) % (2 ** 32)   #mix bits
            a = left_rotate(a, shift[i])        #rotate bits
            a = (a + b) % (2 ** 32)
            a, b, c, d = d, a, b, c
...
def md5(s: bytes) -> bytes:                 #input as bytes, output as bytes
    state = MD5State()
    state.process(BytesIO(s))               #feed in the bytes
    state.finalize()                        #padding
    return state.digest()


def md5_file(file: BinaryIO) -> bytes:
    state = MD5State()
    state.process(file)                     #same thing but passing a file
    state.finalize()
    return state.digest()
